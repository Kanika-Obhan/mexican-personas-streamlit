...
def call_gemini_local(query, previous_conversation, gender, username, botname, bot_prompt, llm_api_key_string, language, max_tokens=256):
    try:
        ...
        generation_config = genai.types.GenerationConfig(
            max_tokens=max_tokens  # Limit response length here
        )
...
# --- Add a slider to control max_tokens ---
st.sidebar.title("⚙️ Response Settings")
max_tokens = st.sidebar.slider("Max response length (tokens)", 50, 1024, 256)
    ...
    if user_question.strip():
        ...
        try:
            ...
            response = call_gemini_local(
                ...
                max_tokens  # Pass the max_tokens value
            )
            end = time.time()
...
if st.session_state.selected_persona and st.session_state.questions:
    st.title(f"{st.session_state.botname} ({st.session_state.bot_origin}) {st.session_state.relationship.title()} Q&Anowbeme") 
    ...
    if st.session_state.bulk_running and not st.session_state.paused:
        if st.session_state.current_question_index < len(st.session_state.questions):
            ...
            response = call_gemini_local(
                ...
                "AIzaSyAWMudIst86dEBwP63BqFcy4mdjr34c87o",
                st.session_state.selected_language,
                max_tokens  # Pass the max_tokens value here
            )
            enddd = time.time()
